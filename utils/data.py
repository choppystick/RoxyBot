...
class Bot(AutoShardedBot):
    ...
    async def on_message(self, msg):
        goodlist = ["good bot", "amazing bot", "cute bot", "best bot", "great bot"]
        if any(x in msg.content.lower() for x in goodlist):
            await msg.channel.send("Thank you ❤")

        badlist = ["trash bot", "useless bot", "dumb bot", "stupid bot", "garbage bot", "horrible bot", "worst bot", "bad bot"]
        if any(x in msg.content.lower() for x in badlist):
            await msg.channel.send("I'm sorry ;-;")
