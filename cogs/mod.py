...
class Moderator(commands.Cog):
    ...
    async def find_playing(self, ctx, *, search: str):
        ...
        await default.prettyResults(
            ctx, "playing", f"Found **{len(loop)}** on y    our search for **{search}**", loop
        )
    ...
    async def _remove_all(self, ctx, search=100):
        ...
        if flag:
            confirmation = "Are you sure you want to delete all the messages here? " \
                           "(THIS COMMAND WILL DELETE ALL MESSAGES. DO NOT PRESS YES UNLESS YOU ARE SURE! IF YOU ARE NOT SURE, IGNORE)"
            msg = await ctx.send(confirmation)

            def check(reaction, user):
                return user == ctx.author and str(reaction) == "👍" and reaction.message == msg
            try:
                await msg.add_reaction("👍")
                await self.bot.wait_for("reaction_add", timeout=10.0, check=check)
                await msg.edit(content="Confirming deleting all message.")

            except asyncio.TimeoutError:
                await msg.delete()
                await ctx.send("Waited too long. Aborting command.")
                flag = False

            except discord.Forbidden:
                await msg.edit("Bot has no reaction permission. This command will be aborted.")
                flag = False
    ...
    @prune.command(name="clear")
    async def _clear(self, ctx, amount):
        """Clear an amount of messages, 4 by default."""
        amount = int(amount)
